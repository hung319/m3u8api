...
app.get('/get-m3u8', async (req, res) => {
    ...
    try {
        ...
        // Tìm link m3u8 ở thẻ span class text-success font-weight-bold
        ...
        // Tìm iframe có link tới .m3u8 hoặc 1 trang chứa player
        const iframeSrc = $2('iframe').attr('src');

        if (!iframeSrc) {
            return res.status(404).send('Không tìm thấy iframe phát video đâu Yuu Onii-chan~');
        }
        ...
        // Tìm link .m3u8 trong script hoặc thẻ video
        ...
        if (match && match[1]) {
            ...
            // Trả ra trực tiếp dạng text/plain dễ lấy nè
...
app.listen(PORT, () => {
    console.log(`Server Mahiro đã chạy ở http://localhost:${PORT} nha Yuu Onii-chan~`);
});
